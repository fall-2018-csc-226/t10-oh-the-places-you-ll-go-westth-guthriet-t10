######################################################################
# Author: Tom West & Thomas Guthrie
# Username: Westth & Guthriet
#
...
def extract_place(file_content):
    ...
    lat = float(file_content.readline().strip("\n"))
    lon = float(file_content.readline().strip("\n"))
    color = file_content.readline().strip("\n")

    # Example: place_tuple = ("Scott's example", "Somewhere special", 41, -10, "black")
    place_tuple = (name, location, lat, lon, color)      # Finish assembling the tuple!
    return place_tuple
